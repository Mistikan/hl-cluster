...
1. Создать разделы под данные с дисков:
    * внешний диск Сахара
    * внешний диск m2
    * данные с ноутбука
1. Переместить данные.
1. Очистить диски, т.к. данные уже перемещены.
1. Подключить все разделы с данными.
1. Написать скрипт, который перемещает торрент файл в заданную директорию с последующим переименовыванием по хешу:
    ```sh
    /data/film.torrent -> /output/hash.torrent
    ```
1. Доработать программу find-torrent-data:
    ```
    А моя программа find-torent-data при поиске должна искать файл не только по размеру и расширению, но чтобы ещё был похож максимально по названию. Тогда вообще идеально.
    ```
1. Наполнить базу данных, чтобы был список файлов для обработки.
1. Натравить find-torrent-data, чтобы она начала искать файлы. Сохранять отчёты в отдельную директорию:
    ```
    /output/hash.torrent -> /report/hash.json
    ```
1. Начать ручное перемещение данных с помощью скрипта.
